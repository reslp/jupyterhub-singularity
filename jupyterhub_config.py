...
class MyCustomSpawner(LocalProcessSpawner):
	def user_env(self, env):
		user_dir = os.path.join(self.notebook_dir, self.user.name)
		env['HOME'] = user_dir
		#env['USER'] = 'notebook_user'  # a non-privileged user in the container
	...
	def make_preexec_fn(self, name):
		def preexec():
			home_dir = os.path.join(self.notebook_dir, name)
...
#c.JupyterHub.authenticator_class = 'myauth.MyCustomAuthenticator'
...
# Set the custom spawner in JupyterHub's configuration
c.JupyterHub.spawner_class = MyCustomSpawner
# Set the directory where user directories will be created
c.MyCustomSpawner.notebook_dir = '/data/homes'
c.MyCustomSpawner.default_url = "/lab/tree/{username}/"
...
#  Default: set()
c.Authenticator.admin_users = {"christoph", "philipp", "martin"}
